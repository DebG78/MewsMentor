-- ============================================================================
-- SEED DATA SCRIPT - SkillPoint Phase 1
-- ============================================================================
...
SELECT
  ...
  jsonb_build_object(
    'capacity', 2,
    'topics_to_mentor', topics,
    'meeting_frequency', 'bi-weekly'
  ),
  'active'
FROM (VALUES
  ('user_001', ARRAY['System Design', 'React', 'Mentoring']),
  ('user_004', ARRAY['Leadership', 'Management', 'Career Growth']),
  ('user_008', ARRAY['DevOps', 'AWS', 'Infrastructure'])
) AS data(user_id, topics)
...
SELECT
  ...
  'active'
FROM (VALUES
  ('user_003', ARRAY['React', 'System Design', 'Testing'], ARRAY['Build confidence in system design', 'Learn testing best practices']),
  ('user_006', ARRAY['Leadership', 'Communication'], ARRAY['Improve presentation skills', 'Learn to lead projects'])
) AS data(user_id, topics, goals)
ON CONFLICT (user_id, program_cohort_id, role_in_program) DO NOTHING;
...
SELECT
  ...
  'Enrolled in the mentoring program to accelerate professional growth',
  '2025-01-15',
  ARRAY['Mentoring']
FROM program_participants
WHERE program_cohort_id = 'cohort_q1_2025';

-- Sample mentoring session event
...
VALUES
  ...
  ('user_004', 'cohort_q1_2025', 'mentoring_session', 'Mentored David on Leadership', 'Discussed presentation techniques and executive communication', '2025-02-05 14:00:00', 'user_006', ARRAY['Mentoring', 'Leadership'], 'David has strong technical skills but needs to work on simplifying complex topics for non-technical audiences.', 4);
...
-- Add skill progress for users based on their growth events
