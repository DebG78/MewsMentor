 ...
 *
 * Creates an .xlsx template with all required columns for the V3 survey format,
 * including Workday enrichment fields, mentee questions, and mentor questions.
 *
 * Run: npx tsx scripts/generateTemplate.ts
 *
 * Output: MewsMentor_Upload_Template.xlsx
 */

import * as XLSX from 'xlsx';

// ============================================================================
// Column definitions matching V3_SHARED_PATTERNS, V3_MENTEE_PATTERNS, V3_MENTOR_PATTERNS
// from src/lib/surveyFieldMapping.ts
// ============================================================================
...
// ============================================================================
// Sample data rows (2 examples: a mentee, a mentor, and a "both" participant)
// ============================================================================
...
const SAMPLE_ROWS = [
  {
    ...
    'What capabilities would you like to develop through mentoring?': 'Strategic thinking, stakeholder management, data-driven decision making',
    ...
    'What is your mentoring goal? (Using the format: I want to...)': 'I want to develop confidence in presenting product strategy to senior leadership and learn how to prioritize across competing business objectives.',
  ...
  },
  {
    ...
    'Why do you want to mentor? What do you hope to get out of it?': 'I want to give back and help others navigate the IC-to-manager transition I went through.',
    ...
    'Share a meaningful impact story from your career': 'Helped a junior engineer grow into a tech lead within 18 months by creating a structured growth plan.',
  ...
  },
  {
    ...
    'Business Title': 'Director of Customer Success',
    'Compensation Grade': 'L2 Director',
    'Location Address - Country': 'India',
    'Org Level 04': 'Customer Success',
    'Org Level 05': 'Enterprise Accounts',
    'Slack User ID': 'U03GHI789',
    'Tell us about your role and focus in a few lines': 'Leading enterprise CS for APAC region. Passionate about customer-led growth and team development.',
    'How would you like to participate as?': 'Both (Mentee and Mentor)',
    // Mentee fields
    'What capabilities would you like to develop through mentoring?': 'Executive presence, board-level communication',
    'Is there a job-specific or role-related area you would like mentoring on?': 'Transitioning from regional to global leadership scope',
    'What is your mentoring goal? (Using the format: I want to...)': 'I want to build executive presence and learn to influence at the C-suite level.',
    'Is there a specific situation or challenge you are navigating?': 'Preparing to take on a global VP role',
    'What kind of mentor help would you like?': 'Sounding board;Strategic advice',
    'Are you open to being mentored by a first-time mentor?': 'No, I prefer an experienced mentor',
    'Session style preference (Mentee)': 'Informal conversation',
    'Feedback style preference': 'Diplomatic but clear',
    // Mentor fields
    'Why do you want to mentor? What do you hope to get out of it?': 'I want to help mid-career professionals build confidence in client-facing leadership.',
    'How many mentees would you like to mentor?': 'One',
    'Is this your first time mentoring?': 'I have mentored before and helped others informally',
    'What support would help you feel confident as a mentor?': 'Conversation guides',
    'Select the capabilities you feel confident mentoring others in': 'Customer success & client relationships;Leadership & management',
    'Is there something specific about your job or field that a mentee could benefit from?': 'Navigating enterprise sales cycles, building exec relationships with key accounts',
    'Share a meaningful impact story from your career': 'Turned around a failing enterprise account by restructuring the CS approach, resulting in 3x expansion.',
    'What do you naturally bring as a mentor? (Pick up to 3)': 'Empathy;Strategic thinking;Real-world examples',
    'Preferred session style (Mentor)': 'Mix of structured and informal',
    'Are there topics or areas you prefer not to be matched on?': '',
    'Is there anything else that would make a match not work?': 'Prefer not to mentor someone from my direct team',
  },
];
...
function generateTemplate() {
  ...
  const templateData = SAMPLE_ROWS.map(row => {
    ...
    for (const col of ALL_COLUMNS) {
      ordered[col] = (row as Record<string, string>)[col] || '';
    }
    return ordered;
  });
  ...
  // Write file
  const outputPath = 'MewsMentor_Upload_Template.xlsx';
  XLSX.writeFile(wb, outputPath);
  console.log(`Template generated: ${outputPath}`);
  console.log(`Columns: ${ALL_COLUMNS.length} total (${SHARED_COLUMNS.length} shared + ${MENTEE_COLUMNS.length} mentee + ${MENTOR_COLUMNS.length} mentor)`);
}
...
function getColumnDescription(col: string): string {
  const descriptions: Record<string, string> = {
    ...
    'What kind of mentor help would you like?': 'Multi-select (semicolons): Accountability partner;Sounding board;Career guidance;etc.',
